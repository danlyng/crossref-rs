...
impl DateParts {
    ...
    pub fn as_date(&self) -> Option<DateField> {
        ...
        fn naive(v: &[Option<u32>]) -> Option<NaiveDate> {
            match v.len() {
                0 => None,
                1 => Some(NaiveDate::from_ymd_opt(v[0]? as i32, 0, 0).unwrap_or_default()),
                2 => Some(NaiveDate::from_ymd_opt(v[0]? as i32, v[1]?, 0).unwrap_or_default()),
                3 => Some(NaiveDate::from_ymd_opt(v[0]? as i32, v[1]?, v[2]?).unwrap_or_default()),
                _ => None,
            }
        }
